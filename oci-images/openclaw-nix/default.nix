...
let
  ...
  sourceInfo = {
    ...
    repo = "openclaw";
    rev = "v${version}";
    sha256 = "sha256-2RNAGMPPzhR3yKedWxMp8gwEnig+N6LmUy8hBE67d9o=";
  ...
  openclawRootfs = pkgs.runCommand "openclaw-rootfs" { } ''
        ...
        cd $out/lib/openclaw
            ...
            LOSSLESS_MANIFEST="$out/lib/openclaw/extensions/lossless-claw/openclaw.plugin.json"
            if [ -f "$LOSSLESS_MANIFEST" ]; then
              chmod u+w "$LOSSLESS_MANIFEST" 2>/dev/null || true
          ${pkgs.python3}/bin/python3 - "$LOSSLESS_MANIFEST" <<'PY'
    import json
    import sys
    ...
    data.setdefault("entry", "./index.ts")
