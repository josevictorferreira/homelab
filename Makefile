...
vmanifests: ## Replace secrets in .enc.yaml manifests using vals.
	@set -euo pipefail; \
	find .k8s-manifests -mindepth 2 -type f \
	  \( -name '*.enc.yaml' -o -name '*.enc.yml' \) \
	  -not -path '.k8s-manifests/flux-system/*' -print0 | \
	while IFS= read -r -d '' f; do \
...
gmanifests: ## Render k8s manifests generated by kubenix.
	...
	find .k8s-manifests -mindepth 1 -maxdepth 1 -type d \
	! \( -name 'flux-system' \) -exec rm -rf {} +; \
	cp -rf result/* .k8s-manifests; \
	rm -rf result; \
	find .k8s-manifests -type f -exec chmod 0644 {} +; \
	find .k8s-manifests -type d -exec chmod 0755 {} +;
